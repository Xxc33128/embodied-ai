...
def valid_trial_folders(root):
    """First valid attempt per trial; interrupted attempts remain on disk."""
    selected = {}
    interrupted = []
    for folder in sorted(root.glob('trial-*')):
        result_file = folder / 'result.json'
        if not folder.is_dir() or not result_file.exists():
            continue
        result = json.loads(result_file.read_text())
        if result['status'] != 'success':
            interrupted.append({'path': folder.name, 'status': result['status'], 'trial_index': result['trial_index']})
            continue
        selected.setdefault(result['trial_index'], folder)
    (root / 'infrastructure-attempts.json').write_text(json.dumps(interrupted, indent=2) + '\n')
...
def audit(root):
    ...
    rows = []
    for folder in valid_trial_folders(root):
        ...
        bank = json.loads((root / 'initializations' / f'trial-{index + 1:02d}' / 'initialization.json').read_text())
        ...
        row = {'trial': index + 1, 'attempt_path': folder.name, 'scene_seed': 2000 + index, 'env_success': result['metrics'].get('success_at_end'), 'calls': result['calls'], 'physics_steps': result['physics_steps'], 'sim_seconds': result['physics_steps'] / 20, 'wall_seconds': round(result['wall_seconds'], 2), 'video_frames': video_frames, 'image_references': images, 'termination': json.dumps(result['termination_reasons'], ensure_ascii=False), 'recording_checks': 'PASS' if not problems else '; '.join(problems), 'reviewed_stage': '', 'reviewed_success': '', 'review_boundary': False}
        review = folder / 'review.json'
        if review.exists():
            data = json.loads(review.read_text())
            boundary = data.get('boundary_pending', False)
            row.update(reviewed_stage=data['stage_max'], reviewed_success='' if boundary else data['stage_max'] == 4, review_boundary=boundary)
        rows.append(row)
    ...
    cards = []
    for row in rows:
        name = row['attempt_path']
        grade = '3–4（终态证据待定）' if row['review_boundary'] else row['reviewed_stage'] if row['reviewed_stage'] != '' else '待复核'
        cards.append(f'<article><h2>Trial {row["trial"]:02d} · seed {row["scene_seed"]}</h2><p>{row["calls"]} 次调用 · {row["physics_steps"]} 控制步 · 仿真 {row["sim_seconds"]:.2f}s · 墙钟 {row["wall_seconds"]:.1f}s</p><video controls preload="metadata" src="{name}/video.mp4"></video><p>录像：front / side / wrist。记录检查：{html.escape(row["recording_checks"])}；审核阶段：{grade}</p><a href="{name}/result.json">原始结果</a> · <a href="{name}/physics.jsonl">物理轨迹</a></article>')
