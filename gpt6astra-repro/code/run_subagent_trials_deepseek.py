"""DeepSeek batch runner. Byte-identical protocol to run_subagent_trials.py; only the
decision-model identity labels differ (deepseek-v4.1-flash-subagent). Prompts, budgets,
...
def chat_response(choice,call):
    return {'id':f'chatcmpl-subagent-{call}','object':'chat.completion',
            'model':'deepseek-v4.1-flash-subagent',
...
def run_trial(args):
    ...
    try:
        ...
        atomic_json(private/'manifest.json',{'protocol':protocol,'scene_seed':scene_seed,'trial_index':args.index,
            ...
            'decision_model':'deepseek-v4.1-flash-subagent','decision_model_snapshot':'unknown; opencode-go subagent backend not independently verifiable','effort_effect':'medium requested in wire; subagent backend effort control not exposed'})
        task=Task(name='robosuite-bowl-deepseek41-subagent',scenes=[Scene(id=f'seed-{scene_seed}',
        ...
        policy=StandardPolicy(model='deepseek-v4.1-flash-subagent',base_url='http://subagent-relay.invalid/v1',
        ...
        outcome={'status':log.status,'metrics':dict(log.results.metrics),
                 ...
                 'source':'deepseek-v4.1-flash persistent trial subagent via LLMAgentPolicy transport',
