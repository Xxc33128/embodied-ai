"""Qwen batch runner. Byte-identical protocol to run_subagent_trials.py; only the
decision-model identity labels differ (qwen3.8-flash-subagent). Prompts, budgets,
...
def run_trial(args):
    ...
    try:
        ...
        atomic_json(private/'manifest.json',{'protocol':protocol,'scene_seed':scene_seed,'trial_index':args.index,
            ...
            'decision_model':'qwen3.8-flash-subagent','decision_model_snapshot':'qwen-token-plan-cn/qwen3.8-flash; pi subagent harness, thinking=medium set by parent harness front-end (not a provider snapshot id)','effort_effect':'medium requested in wire and pi-side thinking level set to medium; provider-side effort control not exposed'})
        task=Task(name='robosuite-bowl-qwen38flash-subagent',scenes=[Scene(id=f'seed-{scene_seed}',
        ...
        policy=StandardPolicy(model='qwen3.8-flash-subagent',base_url='http://subagent-relay.invalid/v1',
        ...
        outcome={'status':log.status,'metrics':dict(log.results.metrics),
                 ...
                 'source':'qwen3.8-flash persistent trial subagent via LLMAgentPolicy transport',
