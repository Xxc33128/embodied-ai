# 原实现 vendored（唯一修改：相对 import 指向本包内 action_edit_kinematics），来源同 commit 的 robodojo_server/kinematics.py。ArmFK/DualKinematics 语义与原文一致，W2 FK 门槛（<2mm/<0.01rad）出自其 check()。
