...
> **版本：v1.3｜2026-09-14**
> 修订记录：v1.3 做文风校准，把 v1.2 中偏口语的表达调回工作文档口径，可读性改动（断句、去套话、少破折号）保留；v1.2 为 humanizer-zh 文风修订。事实、数字、引用与 E 级标注从未改动。v1.1 增补 §2.4 与 §3：用直系前代 Motus v1 的开源代码（`thu-ml/Motus` @ `f771216`，2026-09-14 clone 至 `vla_wam_framework_src/Motus`）做架构级解剖，并与 Motus2 论文口径逐项对差；v1（同日早前版）是纯论文级调研。
>
...
5. 开源状态目前仅限于"仓库已建立"：GitHub 上只有 README 和路线图，代码与权重标注为"9 月内逐步放出"，路线图勾选项均未完成；HF 没有 Motus2 权重；总参数量未披露，已知骨干是 Wan 2.2-TI2V-5B。目前只能做文献级跟踪，复现与迁移评估为时尚早；架构细节已用前代 v1 的开源代码补上（§3，E1）。（E3·一手，2026-09-14 检索）
...
- 监督按轨迹分流：成功轨迹才激活动作监督；失败和次优轨迹里的动作保持干净，只用来当 simulator 和 evaluator 的条件与监督。论文的表述大意是，失败样本可以用于学习动力学和价值，同时避免让策略学到坏动作。（E3·一手，§3.2）
...
> 前提：Motus2 未放码，本节解剖的是 v1 官方代码（`thu-ml/Motus` @ `f771216`，Apache-2.0，本地锚点 `vla_wam_framework_src/Motus`）。Motus 系的骨架由它定义，Motus2 论文自称 "built on Motus"，因此这是当前唯一可复核的架构基线。v2 没说清楚的细节，一律列入待核。
...
- chunk 长度公式是 `action_chunk_size = num_video_frames × video_action_freq_ratio`（`motus.py:86`）。RoboTwin 配置算出来 8 帧 × 2 = 16（`configs/robotwin.yaml:7,13`），HF 模型卡写 48，两处口径不一致，列入待核。
...
`update_obs` 缓存观测，`get_action` 里当前帧先过 T5 文本编码和 VLM（图+指令），再联合去噪，动作进 `action_cache` 逐个执行。预测出的未来帧只用于保存可视化帧网格，不进控制回环（`inference/robotwin/Motus/deploy_policy.py:232-280`，E1）。预测了未来画面却未用于控制，这部分冗余正是 Motus2 用 action-first inference 要省掉的（E3·推断）。官方显存要求（E3，README）：推理超过 24 GB（预编码 T5）或约 41 GB（不预编码）；训练超过 80 GB，A100/H100/B200 级别。
...
长程变体用变长 episode packing 训练（block-diagonal 因果掩码，100 latent-frame 预算，单 micro-batch 最多 8 条）。结果（Find Square + Press Button）：仿真里 global AR 78% 对 hybrid 52%，真机 57.5% 对 25.0%。在这两个探针上，压缩记忆明显不如全历史（E3·一手，§5.5 Table 4）。需注意这是该团队自己设计的探针任务，不宜外推为通用结论。
...
3. 昇腾适配视角（推断）：视频扩散骨干、flow matching、KV cache 窗口重定基、Best-of-N 多候选想象，这套推理成本结构比普通 VLA 重；官方材料中没有任何 NPU 证据，且目前无码可查，暂无法评估（E3·推断）。
