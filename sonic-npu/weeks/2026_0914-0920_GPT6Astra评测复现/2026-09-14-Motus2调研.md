...
> **版本：v1.1｜2026-09-14**
> 修订：v1.1 增补 §2.4 与 §3——基于直系前代 Motus v1 的开源代码（`thu-ml/Motus` @ `f771216`，2026-09-14 clone 至 `vla_wam_framework_src/Motus`）做架构级解剖，并与 Motus2 论文口径逐项对差；v1（同日早前版）为纯论文级调研。
...
>
> 证据口径沿用 [00_总览](<../../reports/00_总览.md>) §3：E1 源码行号、E2 配置实证、E3 文档/论文/官方资料（标注一手的）或明确标注的推断、E4 外审转述未复核。WAM/VLA 概念基础见 [9/8 世界模型背景调研](<../2026_0907-0913_调研与归档/2026-09-08-VLA_WAM_世界模型背景调研报告.md>) §4–§5，本文不重复。**Motus2 本体无码可审**：§3 的 E1 证据全部针对 v1 代码，凡涉及 Motus2 的架构判断均为论文口径（E3）；v1 ≠ v2，差异对照见 §2.3 / §2.4。
...
5. **开源状态：只建了仓库**。GitHub 仅有 README + 路线图（代码/权重"9 月内逐步放出"，尚无任何勾选项）；HF 上无 Motus2 权重；模型总参数未披露（骨干为 Wan 2.2-TI2V-5B）。**当前只能做文献级跟踪，复现/迁移评估为时过早**；架构细节已用直系前代 v1 的开源代码补足（§3，E1）。（E3·一手，2026-09-14 检索）
...
### 2.4 架构机制视角：v1 源码 → Motus2 论文的逐项增量

把两代放在同一张"机制表"里。左列来自 v1 源码（E1，commit `f771216`，file:line 见 §3），右列为 Motus2 论文自述（E3·一手）。
...
| 部署成本 | 每步联合去噪 video+action；预测视频仅存可视化、不进控制（`inference/robotwin/Motus/deploy_policy.py:254,262-278`） | action-first inference：常规控制只跑 policy factor，不生成未来画面 |

**判读（E3·解释）**：Motus2 不是推倒重来，而是在同一族骨架（Wan 2.2 · 30 层 · flow matching · MoT 式头空间拼接）上做四处加法——① 给注意力加阶段化掩码与 value 查询；② 把上下文从"单帧条件"扩成"工作记忆"；③ 外挂触觉专家；④ 把训练从 SFT 扩到"价值模型 + 在线扩散 RL"。下节给出左列每一项的源码依据。

## 3. 代码级解剖：Motus v1 怎么把这套架构落地（E1）

> 前提：Motus2 未放码。本节解剖对象是 v1 官方代码（`thu-ml/Motus` @ `f771216`，Apache-2.0，本地锚点 `vla_wam_framework_src/Motus`）。它定义了 Motus 系的骨架——Motus2 论文明确"built on Motus"，因此这是当前唯一可复核的架构基线；凡 v2 未说明的细节，一律按"待核"处理。
...
- 拼接与切分：`bak/wan/modules/model.py:227-262`；RoPE 只作用于 video 的 Q/K：同文件 `:200-202`。
- 各专家的"块"**只持有自己一侧的参数**（含到 WAN 头空间的投影权重），注意力计算借用 WAN 自注意力（设计注释见 `action_expert.py:216-225`）。
- 两个对迁移/审计重要的推论：① 三模态共享一次全局注意力、**无模态隔离**——这正是 Motus2 要加掩码的地方；② 序列长度 = 视频 token（≈ `3×H/32×W/32`）+ 动作 chunk + VLM 序列（数百 token），单层注意力复杂度对其总和成平方（E1·计算）。

### 3.3 动作专家：1024 维、30 块、与 WAN 逐层 1:1

- 每层 `ActionExpertBlock` 持有：`wan_action_qkv`（形状 `[3, 24, 1024, 128]`）、输出投影 `wan_action_o`、Q/K RMSNorm、自己的 FFN 与 6 参数 AdaLN（`action_expert.py:227-259`）。
- 输入编码分两套：预训练 `ActionEncoder`（纯动作、无 state token）；微调 `StateActionEncoder`（1 个 state token + 动作 chunk）；位置编码为固定正弦（`action_expert.py:87-213`）。
- Register token：微调 4 个、预训练 0 个（`motus.py:569-590`）。
- 解码：AdaLN 调制 + `mlp1x_silu` 动作头（`action_expert.py:262-304`）。
- chunk 长度公式：`action_chunk_size = num_video_frames × video_action_freq_ratio`（`motus.py:86`）。RoboTwin 配置为 8 帧 × 2 = **16**（`configs/robotwin.yaml:7,13`）——注意 HF 模型卡写 48，两者口径不一致，列入待核。
- 官方 README 标称动作专家 ≈641.5M 参数（E3）。

### 3.4 理解专家：把冻结 VLM 接进注意力

- VLM（Qwen3-VL-2B）全程冻结、不截断（`motus.py:529-535`），仅取末层 hidden states（`motus.py:323-330`），经 2048→512 `mlp3x_silu` 适配后进入联合注意力。
- 30 块与 WAN 层数 1:1；无解码器、无位置编码（位置信息由 VLM 内建）（`und_expert.py:108-141`）。
- 实现观察（E1）：推理去噪循环内**每一步都重新调用 `extract_und_features`**（`motus.py:950` 循环前一次，`:973` 又在循环内逐帧重算）——同一 VLM 前向被重复执行，属实现层冗余。Motus2 若已去掉 VLM 分支（§2.4），该开销随之消失；若保留，落地时需修复（E3·推断）。

### 3.5 UniDiffuser 在这套代码里到底是什么

不是某个类，而是**"两个模态各自独立加噪、在同一注意力里联合去噪"**这一训练/推理约定：

- 训练：video 与 action 各自采样独立 timestep/sigma，各自构造 `noisy = x·(1−σ) + ε·σ` 与目标 `ε − x`；首帧 teacher forcing 且其 target 置 0（`motus.py:803-814`）；两路 MSE 按配置权重相加（`motus.py:884-895`，robotwin 权重 1:1，E2）。
- 推理：Euler 循环 t=1→0，视频与动作**在同一循环内同步去噪**（`motus.py:956-1017`）；RoboTwin 默认 10 步（`configs/robotwin.yaml:85`）。
- 仓库另有两版采样器实现（DPM++、UniPC），均为注释状态（`motus.py:1030-1347`）——当前生效的只有 Euler 版。
- 尺寸换算：latent 帧数 `1 + num_video_frames/4`、空间 `H/32 × W/32`（`motus.py:617-619`）；WAN 侧 patch `(1,2,2)`、dim 3072、24 头、30 层、FFN 14336（`bak/wan/configs/wan_ti2v_5B.py:20-25`）。
- **"五模式"的落地程度**：README 声称 UniDiffuser 调度可切换世界模型 / VLA / 逆动力学 / 视频生成 / 联合预测（E3）；但本次检索到的推理实现只有"视频+动作联合去噪"一条路径，未见显式模式开关（E1，检索范围）→ 列入待核。

### 3.6 训练与数据（代码/配置口径）

- 三阶段（README，E3）：Stage 1 只训 VGM（数据金字塔 level 2/3/5）→ Stage 2 三专家 + **潜动作**（level 2–5）→ Stage 3 目标机器人 SFT（level 6）。
- 潜动作是**预计算资产**：数据集只读 `latent_action_dim14/*.pt`（`data/latent_action/latent_action_dataset.py:232-243`，E1）——光流→潜动作的生成脚本未随仓库发布（待核）。
- 优化配置（E2，`configs/robotwin.yaml`）：batch 8、lr 5e-5、weight decay 0.01、warmup 200、线性调度 `f_max 0.99 → f_min 0.4`、grad clip 0.5、时间采样 `logit_normal`；DeepSpeed 提供 zero1/zero2 配置。
- 冻结与参数组（E1，`train/train.py:477-491`）：VLM 冻结；WAN 可配独立学习率（`wan_learning_rate`，robotwin 中为注释态）。

### 3.7 部署链路（以 RoboTwin 为例）

`update_obs` 缓存观测 → `get_action`：当前帧 → T5 文本编码 + VLM（图+指令）→ 联合去噪 → 动作进 `action_cache` 逐个执行；预测出的未来帧**只用于保存可视化帧网格**，不进控制回环（`inference/robotwin/Motus/deploy_policy.py:232-280`，E1）。这个"算了未来画面但控制不用"的浪费，正是 Motus2 用 action-first inference 优化掉的对象（E3·推断）。

官方显存要求（E3，README）：推理 >24 GB（预编码 T5）/ ≈41 GB（不预编码）；训练 >80 GB（A100/H100/B200 级别）。
...
- 用 [DiffusionNFT](https://proceedings.iclr.cc/paper_files/paper/2026/hash/d8e68ddfe22520b45b8fb8d5cbde5a21-Abstract-Conference.html)（本组 ICLR 2026 Oral，前向过程在线扩散 RL）把候选价值转成 flow-matching 策略更新；**只更新动作相关参数，视频骨干与 evaluator 冻结**。`β=0.1`、EMA 0.99、参考策略每 10 次 trainer 更新发布一次、>600 s 陈旧样本丢弃。（E3·一手，论文 §3.3.2 / 附录 E.3）
...
| 复现路径（过渡期） | Motus v1（`thu-ml/Motus`，代码+权重全开）已 clone 至本地锚点 `vla_wam_framework_src/Motus`（@ `f771216`），§3 已据此完成架构解剖 |
...
2. **可作 ① 报告附录 C 的新案例**：附录 C 讨论"VLA/WAM 架构差异对后训练、RL 与昇腾适配的影响"，Motus2 是迄今把**价值模型 + 在线扩散 RL（DiffusionNFT）+ 测试时搜索**塞进单一 WAM 的完整样本；其"动作相关参数才更新、视频骨干冻结"的 MBRL 配方，对讨论"后训练算力构成"有参考价值。§3 的 v1 源码解剖可作为"WAM 落地成本"（注意力结构、每步联合去噪、VLM 重复前向）的实证基线。
...
- [ ] v1 光流潜动作生成脚本未随仓库发布；v2 数据管线中 134 维→20-DoF 重定向有协议描述无代码（论文附录 C/D）。
