...
class VideoWriter:
    ...
    def capture(self, data, sim_time):
        ...
        cam.forward = fwd.astype(np.float32)
        cam.up = up.astype(np.float32)
        self.frames.append(r.render().copy())
        self.last = sim_time
...
def main():
    ...
    manifest = {
        "engine": "mujoco",
        "mode": args.mode, "start_frame": args.start_frame, "history_init": args.history_init,
        "initial_state_source": init_from, "initial_state_npz_sha256": (
            sha256_bytes(args.initial_state) if args.initial_state else None),
