...
def main():
    ...
    if args.fixed_delay_ms not in (0.0, 20.0):
        raise SystemExit(
            f"--fixed-delay-ms 仅允许 0.0 或 20.0，当前 {args.fixed_delay_ms} 非法。"
        )
    ...
    rec = {k: [] for k in ["policy_step", "t", "time_step", "obs(770)", "action(29)", "q_des(29)", "q_des_requested(29)", "q_des_effective(29)", "q_des_effective_substep",
    ...
    for cyc in range(n_cycles):
        ...
        rec["q_des_requested(29)"].append(q_des_pol.astype(np.float32))
        # effective will be filled per physics substep later, for now use q_des_s2s vs effective mapping
        ...
        for _ in range(ppc):
            if delay_fifo is not None:
                ...
                # for P3: record effective per control cycle as last substep effective (approx)
                if cyc==0 and _==0:
                    pass
            else:
                q_des_eff = q_des_s2s
        ...
        # Map effective to policy order for logging (to match requested which is policy order)
        q_des_eff_policy = map_by_name(q_des_eff.astype(np.float64) if delay_fifo is not None else q_des_s2s.astype(np.float64), s2s, policy.joint_names, "eff s2s->pol").astype(np.float32)
        q_des_req_policy = q_des_pol.astype(np.float32)
        rec["q_des_effective_substep"].append(np.stack([q_des_eff_policy]*ppc).astype(np.float32) if delay_fifo is not None else np.stack([q_des_req_policy]*ppc).astype(np.float32))
        rec["q_des_effective(29)"].append(q_des_eff_policy)
    ...
    manifest = {
        ...
        "requested_delay_ms": args.fixed_delay_ms, "measured_delay_ms": float(int(round(args.fixed_delay_ms / (physics_dt * 1000))) * physics_dt * 1000) if args.fixed_delay_ms !=0 else 0.0,
