...
| 日期 | 文档 | 结论 |
|---|---|---|
| 9/17 | [GPT-as-Policy 复现计划对抗性审查](2026-09-17-GPT-as-Policy复现计划对抗性审查.md) | 独立 agent 有条件通过；3 项 P2：Franka 支持臂与 250 Hz 子步、异常/重试统计、正式 NPU 随机流；原计划保持不变，权重聚合哈希元数据与原记录一致 |
| 9/17 | [GPT-as-Policy 在 MuJoCo 与昇腾上的复现计划](2026-09-17-GPT-as-Policy在MuJoCo与昇腾上的复现计划.md) | 按用户要求保留原任务/双 ARX X5/权重/完整 agent 链路，只迁移仿真与推理后端；50 布局、75 源文件哈希匹配，9 支持轨迹元数据匹配；叠衣服等难点明确列项，尚未实施迁移 |
