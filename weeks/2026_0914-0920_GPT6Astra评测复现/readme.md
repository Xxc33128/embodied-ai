...
| 日期 | 文档 | 结论 |
|---|---|---|
| 9/17 | [图像空间与深度理解后训练生态调研](2026-09-17-图像空间与深度理解后训练生态调研.md) | 网络调研(全 E3):四条后训练路线(合成 VQA SFT / GRPO 可验证奖励 RL / 深度-3D 先验注入 / VLA 动作侧对齐)+ 训练数据集清单(RoboSpatial 等)+ 框架选型(verl/ms-swift 原生多模态 GRPO,与 9/8 框架调研衔接);风险集中在评测污染与合成-真实 gap,尚未做本地源码核读;v1.1 文风修订(论据与证据等级不变) |
