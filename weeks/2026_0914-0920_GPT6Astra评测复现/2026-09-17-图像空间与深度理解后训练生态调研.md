...
> **版本:v1.1 · 2026-09-17**(v1.1 文风修订:精简公式化表达与冗余强调;论据与证据等级不变)
...
1. **空间与深度缺失源于训练范式,和模型容量无关。**主流 VLM 的视觉编码器以 2D 语义对比学习(CLIP 系)为主,caption/VQA 语料几乎不含度量几何;有工作实证深度信息在 VLM embedding 中"线性不可读"(embedding-space depth injection 实验,E3,arXiv:2607.11173)。补这块能力要靠后训练注入新的监督信号,加参数解决不了。
2. **四条算法路线,按"监督信号从哪来"划分。**A. 合成空间 VQA 数据做 SFT(数据量大、成本低,上限受合成质量约束);B. 可验证奖励 RL/GRPO(空间答案天然可自动判分,2025 年主流);C. 深度/3D 先验直接注入输入端或表征(最直接对准"深度",需改数据管线或结构);D. VLA 动作侧空间对齐(空间能力以动作成败为最终出口)。
3. **路线 A 是奠基,路线 B 是目前性价比最高的增量。**SpatialVLM(CVPR'24,2B 条 VQA)确立"2D 反推 3D → 生成 QA → SFT"范式;2025 年起同一批数据换成 GRPO 训练,普遍比 SFT 再高数个百分点(STAR-R1 报 +23%,SpatialThinker 报 +5.5% vs SFT,均 E3)。
4. **"学到深度"最直接的是路线 C。**SpatialBot 直接把深度图当第二输入;SD-VLM 把深度编码为前缀 token;SpatialRGPT 用区域级深度。三者都证明:不注入几何信号,纯文本 QA 很难学到度量深度。
5. **数据侧不缺燃料。**开源训练集从 45 万条(SpaRE)到千万级(InternSpatial)都有;机器人向有 NVIDIA RoboSpatial(100 万图、5000 个 3D 扫描、300 万空间关系标注)。瓶颈已经从数据量转移到合成-真实 gap 与评测污染。
...
1. **定性空间关系**(qualitative):左右/上下/前后、包含、相邻、朝向。2D 平面关系(图片里的左右)与 3D 场景关系(世界坐标系里的前后)经常冲突,VLM 的左右混淆、"近处大物体在后面"类错误多出自这里。
...
后训练要做的,就是引入含几何真值(或伪真值)的新监督信号。具体分四条路线,见下节。
...
四条路线经常组合使用:A 产出的数据集可以直接喂 B;C 的深度 token 也能嵌进 D 的 VLA。时间线上,A 集中在 2024,C 在 2024–2025 持续,B 自 2025 年起爆发(DeepSeek-R1 带动 RLVR 范式),D 从 2025 下半年起密集出现。
...
空间 QA 的答案(坐标、相对方向、深度排序、格子位置)可以用规则自动判分,既不需要人工偏好标注,也因为答案对错客观而很难被奖励 hacking。这让它很适合 DeepSeek-R1 式的 RLVR:rollout → 规则打分 → GRPO 更新,无需单独训练 reward model。
...
- **数据**:STVQA-7K,仅 7,587 条基于 Visual Genome 场景图合成的空间 VQA。路线 A 动辄百万级,这里七千多条就够,可见 RL 的样本效率。
...
- 奖励仍是答案层面的判分,度量深度的连续误差(差 0.3m 还是差 3m)通常没有被细粒度奖励利用(SpatialThinker 的 grounding 分量是少数例外);
- Long CoT 训练成本高、推理变慢;
- 与路线 A 共享评测污染风险。
...
- **MindJourney**(NeurIPS'25):测试时优化 VLM 隐状态,借冻结视频扩散世界模型推动隐状态沿相机轨迹移动,相当于在想象中"绕到物体后面看",零训练接近 SOTA(E3)。
...
选型上,对已有 VLA(如 GR00T 系),侵入最小的是 Spatial Forcing / InSpire 类(不动骨干);SpatialVLA 的 Ego3D 编码是架构级方案,上限更高,但要重训。
...
**机器人向首选 RoboSpatial**(2D 自我中心图像与 3D 扫描配对,正是路线 A 管线在机器人域的成品);通用任务选 InternSpatial;RL 起步选 VSI-100k。
...
与本工作区资产的衔接:`vla_wam_framework_src/` 已有 verl 与 RLinf 源码锚点(9/8 周调研)。路线 B 的论文级配方(SVQA-R1/SpatialThinker 类)大致等于 verl 多模态 GRPO 加自定义奖励函数,迁移成本主要在数据管线(VQASynth 式 3D 反推),不在训练框架。昇腾上的可行性,verl 的设备抽象与 RLinf 的 Ascend CI 经验(9/8 报告 §7.2/§9.4)已有结论,此处不重复。
...
- 方法:走 SpatialBot 路线,深度图当第二输入,用 SpatialQA(或 RoboSpatial 子集)做 SFT。
- 工具:LLaMA-Factory / ms-swift,LoRA 即可。
- 注意:需要为每帧配深度(传感器优先,否则 DepthAnything 估计)。

**场景 2:想要推理型空间能力(CoT、多步、可泛化)**
- 方法:先 A 后 B。合成 QA 做轻量 SFT,再上 GRPO(奖励参考 SpatialThinker 四分量或 SVQA-R1 视角一致)。
- 工具:verl(或 ms-swift)。
